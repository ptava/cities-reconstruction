...
The shapefiles stage keeps `plan()` and `run()` as its stable public facade while delegating pure diagnostic summaries to `shapefiles_diagnostics.py`, generated Markdown reports to `shapefiles_reporting.py`, self-contained HTML/SVG feedback to `shapefiles_rendering.py`, and ordered artifact assembly plus manifest-last publication to `shapefiles_publication.py`. Retrieval, classification, and artifact file writes remain in the stage facade until their own behavior-preserving decomposition slices.
