...
3. `point_cloud`: prepare separate City4CFD ground and building PLY point clouds from DTM/DSM rasters or supplied cloud pairs, project building footprints into the metric CRS, and write alignment diagnostics.
...
Geospatial transformations are centralized in `geometry/crs.py`, the boundary around PROJ/pyproj for horizontal CRS validation and conversion with fixed GIS X/Y ordering. It also retains the small-ROI helpers and Florence compatibility entry points. `geometry/crs_inputs.py` resolves dataset declarations against `.prj` metadata without importing configuration or stages. Configuration selects the working CRS; stages own input policy and contextual errors. There are no stage-local EPSG conversion formulas or Florence datum-shift constants.

Maintained geospatial readers were evaluated separately from that numerical refactor. [Pyogrio supports ESRI Shapefile through GDAL](https://pyogrio.readthedocs.io/en/latest/supported_formats.html), and [Rasterio reads GDAL-supported raster formats](https://rasterio.readthedocs.io/en/stable/quickstart.html), including GDAL's built-in [AAIGrid driver](https://gdal.org/en/stable/drivers/raster/aaigrid.html). Neither is adopted in the current dependency set: replacing the existing readers would add a GDAL packaging/runtime surface and requires a separate fixture-parity change that preserves deterministic validation, paired-tile handling, provenance, and established diagnostics.

Stage packages preserve their established public entry points, including `plan()` and `run()` for executable stages, while implementation modules remain stage-internal. The shapefiles package delegates raw Overpass transport/cache handling, ESRI SHP/DBF decoding, and WMS evidence retrieval to `stages/shapefiles/inputs.py`; Overpass tag inventory, classification, payload-to-GeoJSON transformation, and shared local-polygon conversion to `stages/shapefiles/transformation.py`; supplemental ESRI validation, source-to-WGS84 conversion through the shared adapter, record-to-feature transformation, and tree-attribute mapping to `stages/shapefiles/supplemental.py`; cross-source tree deduplication and configured polygon surface-precedence/overlap resolution to `stages/shapefiles/policy.py`; pure diagnostic summaries to `stages/shapefiles/diagnostics.py`; generated Markdown reports to `stages/shapefiles/reporting.py`; self-contained HTML/SVG feedback to `stages/shapefiles/rendering.py`; and ordered artifact assembly plus manifest-last publication to `stages/shapefiles/publication.py`. `stages/shapefiles/stage.py` retains orchestration and owns the stage lock; both it and the input adapter publish stage-owned text, JSON, GeoJSON, WMS evidence, reports, and previews through same-directory atomic replacement. Supplemental tree mappings are centralized in `TREE_ATTRIBUTE_MAPPINGS`: adding a recognized alias group does not require changing the record conversion flow, while every original DBF field remains available under `source_attributes` even when it has no normalized tree tag. Metric mappings recognize `mm`, `cm`, or `m` either in the value (for example, `32 cm`) or in the matched attribute name (for example, `dbh_cm`). Explicit declarations must agree; otherwise the derived metric tag is omitted. Unitless diameter/DBH values follow the standard mapping default of centimetres, while circumference and other metric values default to metres. No magnitude-based unit guessing is used.

`StageId` provides stable stage identity independently of presentation order. The dependency-neutral stage-layout catalogue stores only each identity and sequence number; its `number_name` property composes directory names such as `03_point_cloud` from those two values. `StageSpec` adds maturity, automatic-selection policy, dependencies, inputs, stage-owned CLI options, planners, and execution adapters without duplicating layout metadata. The CLI derives executable choices, dependency-aware `run` plans, stage-focused `run-stage` argument registration, and dispatch from that registry; stage-specific CLI overrides are applied by the focused runtime adapters in `stage_runtime.py`. Use `cities-reconstruction run-stage STAGE --help` to see only the overrides accepted by that stage; dependency-aware `run --help` lists the combined override surface needed by execution plans.

The point-cloud package delegates building-footprint GeoJSON reading, canopy PNG decoding, and paired ESRI ASCII-grid discovery/parsing to `stages/point_cloud/inputs.py`; strict vertex-only ASCII PLY parsing, horizontal conversion and ROI selection to `stages/point_cloud/cloud_inputs.py`; projected-polygon predicates, spatial indexing, DTM/DSM scanning, and building/tree/unclassified point classification to `stages/point_cloud/geometry.py`; alignment status, shift scoring, and the complete diagnostics payload to `stages/point_cloud/diagnostics.py`; Markdown generation to `stages/point_cloud/reporting.py`; self-contained HTML plus browser scene-data preparation to `stages/point_cloud/rendering.py`; and ordered artifact assembly plus manifest-last publication to `stages/point_cloud/publication.py`. `stages/point_cloud/stage.py` retains input-policy validation and handoff selection, CRS projection, artifact writes, orchestration, and its public `plan()`/`run()` facade.
...
| `point-cloud` | `implemented` | Paired DTM/DSM grids or a ground/building PLY pair, plus building-footprint GeoJSON; Stage 1 is only the default footprint producer | `--building-footprints-geojson`, `--tree-canopy-overlay` | `03_point_cloud` |
...
The `shapefiles` stage permits only one active writer through `01_shapefiles/.stage.lock`. After acquiring that lock it removes the previous manifest, retains reusable Overpass caches, publishes each replacement file atomically, and writes the new manifest last. If retrieval, transformation, or publication is interrupted, prior individual files remain intact until their replacements are complete and no stale manifest advertises the mixed directory as a trusted handoff.
...
The workflow in `.github/workflows/quality.yml` runs on every push and pull request with Python 3.11, a lockfile-verified development environment, read-only repository permissions, and dependency caching. Ruff checks the complete production package plus the maintained test boundary listed in `pyproject.toml`, and configured mypy checks the complete `cities_reconstruction` package. Raw TOML tables are validated and narrowed to string-keyed mappings before field-specific parsing builds the typed application configuration. Urban-planning GeoJSON is narrowed from decoder-owned `object` values into validated FeatureCollection, Feature, properties, Point geometry, and coordinate types before normalized features enter Stage 1; a focused mypy rule prevents explicit `Any` from returning at that boundary. Coverage measures the complete package with branch tracking and enforces an 80% floor, 5.73 percentage points below the lowest maintained multi-runtime result of 85.73% and 7.62 points below the current Python 3.11.12 result of 87.62%.
...
- region name and WGS84 center latitude/longitude; an optional advanced `reconstruction.working_crs` override
...
- zero or more uniquely named `[[shapefiles.supplemental]]` municipal ESRI shapefiles with required path and existing-data category, plus a CRS declaration or readable `.prj`; `group_tag` is required for every non-tree category and prohibited for trees, while `enabled` is optional
- zero or more uniquely named `[[urban_planning.inputs]]` externally authored mixed Point GeoJSON files with independent source CRS declarations (WGS84 by default)
...
Use `[[shapefiles.supplemental]]` only for municipal datasets that complete existing OSM information. Surface categories require Polygon/MultiPolygon records; `category = "trees"` requires Point/MultiPoint records. `name`, `path`, and `category` are required; `source_crs` is needed only without a readable `.prj`. Names are unique, and `enabled` defaults to `true`. `group_tag` is required for every non-tree category and is prohibited when `category = "trees"`.

```toml
[[shapefiles.supplemental]]
name = "florence_streets"
path = "../data/street_areas.shp"
source_crs = "EPSG:3003"
category = "roads"
group_tag = "roads"
enabled = true
...
Each file is one GeoJSON `FeatureCollection`. Every feature requires a two-dimensional finite Point, a safe `properties.id`, `properties.kind` equal to `tree` or `air_purifier`, and an exact internal parametric `properties.model` name. EPSG:4326 coordinates use `[longitude, latitude]`; EPSG:3857 coordinates use Web Mercator `[x, y]` metres. Other declared CRSs use GIS `[x, y]` ordering; Stage 1 normalizes all accepted inputs to EPSG:4326.
...
The core `air_purifiers.run(...)` stage consumes only the normalized `01_shapefiles/air_purifiers.geojson`. It validates identity and provenance, retains normalized EPSG:4326 source coordinates alongside projected/local coordinates, resolves model defaults and overrides, applies anisotropic scaling and counter-clockwise +Z rotation, and translates each model into the same local working-CRS frame as City4CFD and trees. With terrain configured, all four rotated footprint corners must be inside its extent before sampling; without terrain, bases use `z=0`.
...
### Coordinates and dataset CRS

Start with the location, then the data. Enter `center_lat` and `center_lon` in **WGS84 decimal degrees**, plus the ROI diameters in metres. The map's display projection does not change that input contract: copy its reported latitude/longitude, not tile coordinates. There is no CRS setting under `[region]`.

Choose one elevation route: DTM **and** DSM rasters, or already classified ground **and** building PLY clouds. The application selects a projected metre working CRS from the rasters, or retains a suitable prepared-cloud CRS. Without either, or when the cloud source is unsuitable as a working plane, it selects WGS84 UTM from the ROI center. Supplemental and planning files do not select the working CRS. A shapefiles-only run can omit elevation inputs; point-cloud preparation cannot.

Normally leave `[reconstruction]` out. Advanced users may set `working_crs = "EPSG:25832"` in that table (`"auto"` is equivalent to omission). **`working_crs` is the target horizontal CRS for City4CFD: vector inputs and prepared point clouds are transformed to it, while DTM/DSM must already use it because raster reprojection is not yet supported.** It never declares an input dataset's existing coordinates. An explicit target must suit the site: metre units alone do not guarantee low distortion. Geographic, geocentric, vertical, non-metre and Web Mercator working CRSs are rejected.

Each dataset keeps its own source CRS:

| Input | Source coordinates | Preparation |
| --- | --- | --- |
| ROI center and OSM | WGS84 longitude/latitude | Stage 1 GeoJSON remains WGS84; downstream geometry is projected to the working CRS. |
| DTM / DSM `.asc` | Adjacent `.prj` or `inputs.dtm_source_crs` / `inputs.dsm_source_crs` | Must already share the working CRS and an aligned grid; no raster resampling. |
| Ground / building `.ply` | Explicit `inputs.point_cloud_source_crs` | Transform X/Y when needed; preserve Z and supplied classes. |
| Supplemental `.shp` | Adjacent `.prj` or that input's `source_crs` | Normalize through PROJ to Stage 1 WGS84; retain the source definition. |
| Urban-planning GeoJSON | That input's `source_crs`, default WGS84 | Normalize to Stage 1 WGS84 independently of the other files. |

For rasters and shapefiles, declarations and metadata must agree. Supply a source declaration only from the dataset's documentation; changing it does **not** reproject the file. Unknown raster CRS always fails, even with an explicit working override. `.prj` evidence is rechecked at input use and included in fingerprints. A new supplemental path supplied through `--streets-shapefile` or `--green-areas-shapefile` uses its own `.prj` or an explicit matching `--*-shapefile-crs`; it does not inherit the working CRS or the replaced file's CRS.

For Florence, enter the location and retain the example's `dtm_source_crs` / `dsm_source_crs` declarations. They describe the supplied rasters; **EPSG:25832 is selected automatically**, without a second working-CRS entry. Municipal EPSG:3003 shapefiles and WGS84 planning points coexist normally: each is interpreted in its own source CRS, then normalized for the shared workflow. The bundled `.prj` datum definition is respected.

Before processing, run `uv run cities-reconstruction validate-config --config config/examples/florence.toml`. The coordinate plan names the selected input route, working CRS and selection reason, each dataset's source CRS and evidence (metadata, declaration or GeoJSON default), and the planned conversions. Human `dry-run`, `run-stage` and `run` also show this interpretation. Add `--json` for structured `coordinate_plan` data: validation returns an object; dry-run keeps its list of stage plans; execution keeps its result fields and adds the plan. Validation inspects coordinate metadata, not every raster cell or PLY vertex; stage checks and graphical previews remain necessary.

Migration: remove `region.crs`; move it to `[reconstruction].working_crs` only if an explicit output override is genuinely needed. Rename input `dtm_crs`, `dsm_crs` and `point_cloud_crs` to the corresponding `*_source_crs` fields. Supplemental/planning `crs` remains a documented legacy alias for `source_crs`; supplying both is an error. Never migrate the old working setting into missing dataset declarations without verifying the dataset's actual CRS.

Code ownership follows the same sequence: `config.py:load_settings()` parses typed settings without dataset I/O; `geometry/crs_inputs.py` reads source evidence; `geometry/spatial_plan.py` resolves an immutable plan; stages consume `config.working_crs` and `config.coordinate_plan`. `load_config()` composes parsing and preparation for Python callers. The requested override stays separate from the resolved value. After changing coordinate-related settings programmatically, call `prepare_config()` and inspect the new plan; runtime verification rejects changed raster evidence rather than silently selecting a new frame. `geometry/crs.py` owns PROJ operations, not workflow policy.

To use prepared clouds instead of rasters, replace the raster input entries with:

```toml
[inputs]
# Keep the other required input settings from the full example.
ground_point_cloud_path = "data/ground.ply"
building_point_cloud_path = "data/buildings.ply"
point_cloud_source_crs = "EPSG:32633"  # The actual source CRS, not the desired target.
```

Both files must be vertex-only `format ascii 1.0` PLY with scalar `x`, `y`, `z` properties. Additional scalar properties are ignored in the XYZ output. Coordinates must be absolute/georeferenced, not a cloud shifted to an undocumented local origin. Samples are clipped to the projected ROI bounding box; Z remains unchanged apart from the existing millimetre output precision. The source and working CRSs can differ. Do not combine this mode with DTM/DSM or a canopy overlay. The preview identifies these as **user-classified** clouds; it does not claim to verify classification or provide independent DSM alignment evidence. DSM completeness is reported as not applicable, and the unclassified diagnostic cloud is empty.

All elevation inputs must already use compatible metre heights and vertical references: no vertical datum conversion is performed. Changing the working CRS or ROI center requires rerunning affected producers. City4CFD rejects point clouds with a missing/different producing CRS. Generated-terrain consumers additionally require the recorded local XY origin to match within 0.001 m, so an old mesh from a different center is rejected even under the same CRS. External terrain meshes must already use the application's local working frame; they are not georeferenced or transformed automatically.

## Point Cloud Outputs

The `point-cloud` stage reads DTM/DSM ASCII grids or a supplied ground/building PLY pair and, by default, the authoritative Stage 1 footprints at `<output.root_directory>/01_shapefiles/buildings.geojson`, then writes outputs under `<output.root_directory>/03_point_cloud`. It never activates files under the dormant `02_visual_enrichment` directory merely because they exist. A deliberate one-run alternative requires `--building-footprints-geojson`; the selected path is preserved in diagnostics, manifests, and reports.

- `ground_points.ply`: ground point cloud for City4CFD
- `building_points.ply`: building point cloud for City4CFD, sampled from DSM cells at least 2 m above DTM and inside building footprints, or transformed from the supplied building cloud
...
- `building_footprints_epsg25832.geojson` for Florence, or `building_footprints_projected.geojson` for other working CRSs: projected footprints in the same metric CRS as the PLY files
...
- `surface_layers/`: working-CRS polygon copies of the non-empty stage-1 surface categories, imported as named City4CFD `SurfaceLayer` entries; source coordinates are validated as EPSG:4326, projected, and clipped to the configured circular outer ROI so crossing-feature boundaries can be imprinted reliably
...
The tree output directory permits only one active writer through `.stage.lock`. The stage invalidates current and legacy manifests only after acquiring that lock, publishes every STL, JSON, report, and preview through same-directory atomic replacement, removes stale per-species crown STLs under its existing allowlisted `*.stl` cleanup policy, and publishes `manifest.json` last. An interrupted run therefore releases the lock, leaves no trusted manifest, and preserves the previous version of any file whose atomic replacement did not complete.
...
The preview surfaces are not final CFD-ready City4CFD outputs. They exist so users can inspect coordinate scale, footprint placement, preserved holes, rough building geometry, terrain triangulation, and actual generated mesh triangles before and after running the external reconstruction tool. The tree STLs are translated to the same local frame used by the City4CFD handoff, while the placement GeoJSON stays in the projected working CRS. True LoD2.2 roof geometry is expected to come from City4CFD/roofer using the projected roofprint polygons and the building point cloud.
...
- `air_purifier_placements.geojson`: working-CRS points plus local placements, base elevations, selected models, target/native dimensions, scale factors, normalized rotations, parameter provenance, source coordinates, input IDs, and ROI zones
