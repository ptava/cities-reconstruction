...
- Source baseline: point-cloud input-adapter extraction is the current review slice, with proposed subject `refactor: extract point-cloud input adapters`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: point-cloud Markdown generation lives in `stages/point_cloud/reporting.py`; self-contained HTML and browser scene-data preparation, including voxel preview sampling, live in `stages/point_cloud/rendering.py`. Commit: `259d40b refactor: extract point-cloud presentation`.
- Current review slice: `stages/point_cloud/inputs.py` owns building-footprint GeoJSON reading, canopy PNG decoding, and ESRI ASCII-grid metadata, discovery, pairing validation, value reading, and paired-cell iteration. `stage.py` retains input-policy validation and handoff selection, CRS projection, raster-to-point geometry/classification, diagnostics, artifact writes, manifest publication, and public `plan()`/`run()` behavior. The behavior-preserving verification passes 461 tests with 86.04% branch coverage on Python 3.11.12; Ruff passes, configured mypy passes for 17 source files, and full-package mypy passes for 43 source files. `.codegraph/` remains intentionally untracked.
...
| 3 | Break up god modules | In progress | The shapefiles decomposition is complete. Point-cloud input adapters, reporting, and HTML/scene-data rendering now live in focused sibling modules, reducing `point_cloud/stage.py` to about 1,210 lines; `city_models/stage.py` remains 2,067. Preserve public `plan()`/`run()` facades while continuing point-cloud geometry/classification, diagnostics, and publication slices. |
...
Continue Checkpoint 4 with a behavior-preserving extraction of point-cloud geometry and raster-to-point classification. Keep input adapters, diagnostics, publication, external behavior, and public `plan()`/`run()` stable. Do not mix later point-cloud boundaries, Checkpoint 5 transactional publication, or Checkpoint 8 shared CRS centralization into that slice. Continue the remaining point-cloud decomposition before starting `city_models/stage.py`.
