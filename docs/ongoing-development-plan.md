...
- Source baseline: City4CFD footprint overlap and inner-ring diagnostics extraction is complete in the local `HEAD` commit with subject `refactor: extract City4CFD diagnostics`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: `stages/city_models/diagnostics.py` owns footprint geometry normalization for diagnostics, bounding-box screening, overlap measurement and ordering, inner-ring counting, and the complete diagnostic payload. `stage.py` calls `build_footprint_diagnostics()` after City4CFD execution and still owns input/handoff validation, preview geometry, artifact writes/publication, orchestration, and public `plan()`/`run()` behavior. Commit: local `HEAD`, `refactor: extract City4CFD diagnostics`.
- Verification baseline: the behavior-preserving suite passes 466 tests with 86.08% branch coverage on Python 3.11.12; Ruff passes, configured mypy passes for 17 source files, and full-package mypy passes for 49 source files. `.codegraph/` remains intentionally untracked.
...
| 3 | Break up god modules | In progress | The shapefiles and point-cloud decompositions are complete. City4CFD presentation and footprint diagnostics now live in focused sibling modules, reducing `city_models/stage.py` from 2,067 to 1,277 lines. Preserve public `plan()`/`run()` facades while continuing City4CFD publication next. |
...
Status: the planned focused shapefiles and point-cloud slices are complete. City4CFD presentation and footprint diagnostics extraction are also complete; City4CFD publication is next.
...
- Completed City4CFD diagnostics slice: `stages/city_models/diagnostics.py` owns footprint geometry normalization for diagnostics, bounding-box screening, overlap measurement and ordering, inner-ring counting, and the complete diagnostic payload. `stage.py` calls `build_footprint_diagnostics()` after execution, writes the artifact, and retains input/handoff validation, preview geometry, manifest publication, stable orchestration, and public `plan()`/`run()` behavior. Commit: local `HEAD`, `refactor: extract City4CFD diagnostics`.
- Remaining Checkpoint 4 scope: extract City4CFD publication before input adapters/parsers and domain transformations/validation in later focused slices.
...
Continue Checkpoint 4 with a behavior-preserving extraction of ordered artifact assembly, metrics/details construction, and manifest-last publication into `stages/city_models/publication.py`. Keep input/handoff validation, City4CFD configuration/execution, preview-mesh geometry, artifact writes, external behavior, and public `plan()`/`run()` stable. Keep input adapters/parsers, domain transformations/validation, Checkpoint 5 transactional-publication expansion, and Checkpoint 8 shared CRS centralization in later focused slices.
