...
- Source baseline: City4CFD Markdown/HTML and mesh scene-data presentation extraction is complete in the local `HEAD` commit with subject `refactor: extract City4CFD presentation`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: `stages/city_models/reporting.py` owns Markdown handoff reporting and semantic surface-layer status text; `stages/city_models/rendering.py` owns self-contained HTML, OBJ parsing, bounded mesh sampling, scene recentering, semantic-layer colors, and browser scene-data preparation. `stage.py` retains input/handoff validation, City4CFD configuration/execution, footprint diagnostics, QA geometry and artifact writes, manifest publication, orchestration, and public `plan()`/`run()` behavior. Commit: local `HEAD`, `refactor: extract City4CFD presentation`.
- Verification baseline: the behavior-preserving suite passes 465 tests with 86.12% branch coverage on Python 3.11.12; Ruff passes, configured mypy passes for 17 source files, and full-package mypy passes for 48 source files. `.codegraph/` remains intentionally untracked.
...
| 3 | Break up god modules | In progress | The shapefiles and point-cloud decompositions are complete. City4CFD Markdown reporting and HTML/mesh scene-data rendering now live in focused sibling modules, reducing `city_models/stage.py` from 2,067 to 1,340 lines. Preserve public `plan()`/`run()` facades while continuing City4CFD diagnostics next. |
...
Status: the planned focused shapefiles and point-cloud slices are complete. City4CFD Markdown/HTML and mesh scene-data presentation extraction is also complete; footprint diagnostics are next.
...
- Completed City4CFD presentation slice: `stages/city_models/reporting.py` owns Markdown handoff reporting and semantic surface-layer status text. `stages/city_models/rendering.py` owns self-contained HTML, OBJ parsing, bounded mesh sampling, scene recentering, semantic-layer colors, and browser scene-data preparation. `stage.py` retains input/handoff validation, configuration/execution, footprint diagnostics, deterministic QA geometry and artifact writes, manifest publication, stable orchestration, and public `plan()`/`run()` behavior. Commit: local `HEAD`, `refactor: extract City4CFD presentation`.
- Remaining Checkpoint 4 scope: extract footprint diagnostics before City4CFD publication, input adapters/parsers, and domain transformations/validation in later focused slices.
...
Continue Checkpoint 4 with a behavior-preserving extraction of footprint overlap and inner-ring diagnostics into `stages/city_models/diagnostics.py`. Keep input/handoff validation, City4CFD configuration/execution, preview-mesh geometry, artifact writes/publication, external behavior, and public `plan()`/`run()` stable. Keep City4CFD publication, input adapters/parsers, domain transformations/validation, Checkpoint 5 transactional-publication expansion, and Checkpoint 8 shared CRS centralization in later focused slices.
