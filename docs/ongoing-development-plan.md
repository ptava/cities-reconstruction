...
Status: complete. The planned focused shapefiles, point-cloud, and City4CFD decompositions preserve each stage package's public facade.
...
- Completed City4CFD publication slice: `stages/city_models/publication.py` owns `CityModelsPublicationInput`, stable artifact ordering and conditional handoff flags, surface-layer and execution details, metrics assembly, and the final manifest publication call. `stage.py` invokes `publish_city_models_manifest()` after every artifact write and retains input/handoff validation, execution, preview geometry, stable orchestration, and public `plan()`/`run()` behavior. Commit: `8ee70b7 refactor: extract City4CFD publication`.
- Completed City4CFD input-boundary slice: `stages/city_models/inputs.py` owns polygon FeatureCollection and JSON-object parsing plus two-metre ASCII PLY elevation-cell aggregation. `stage.py` retains completed-manifest and named-artifact handoff policy, failed-alignment rejection, configuration/execution, preview geometry, artifact writes, stable orchestration, and public `plan()`/`run()` behavior. Commit: `6e51b82 refactor: extract City4CFD input adapters`.
- Completed City4CFD domain-geometry slice: `stages/city_models/geometry.py` owns surface-layer projection/clipping, fallback preview triangle construction, point-cloud elevation rules, the stage-local EPSG:25832 conversion implementation, and successful core-geometry validation. `stage.py` retains supported-CRS decisions, all artifact file writes, handoff policy, configuration/execution, stable orchestration, and public `plan()`/`run()` behavior. Commit: local `HEAD`, `refactor: extract City4CFD geometry`.
