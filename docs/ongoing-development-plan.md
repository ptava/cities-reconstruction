...
- Source baseline: captured after the `refactor: extract shapefiles cross-source policy` code-and-plan commit; inspect the live Git history for its hash.
...
- Most recently completed slice: the seventh behavior-preserving `shapefiles.py` decomposition slice delegates cross-source tree deduplication and configured polygon surface-precedence/overlap resolution to `shapefiles_policy.py`. Shared GeoJSON-to-local-polygon conversion moves to `shapefiles_transformation.py`; orchestration, gap filling, artifact writes, compatibility aliases, and public `plan()`/`run()` remain stable in `shapefiles.py`. `.codegraph/` remains intentionally untracked.
- Verification baseline for the cross-source-policy review slice: Ruff clean; configured mypy clean across 17 source files; full-package mypy clean across 33 source files; 449 tests passed; branch coverage 85.88%; `git diff --check` clean.
...
| 3 | Break up god modules | In progress | `shapefiles.py` is reduced to 809 lines after extracting pure diagnostics, raw input adapters/parsers, publication orchestration, HTML/SVG rendering, Markdown reporting, Overpass and supplemental transformation, and cross-source policy. `point_cloud.py` remains 2,100 lines and `city_models.py` 2,067. Preserve public `plan()`/`run()` facades while continuing focused slices. |
...
Continue Checkpoint 4 with a behavior-preserving extraction of point-cloud Markdown reporting and HTML/scene-data rendering into focused modules. Keep `point_cloud.py` input validation, raster processing, diagnostics, publication, external behavior, and public `plan()`/`run()` stable. Do not mix later point-cloud boundaries, Checkpoint 5 transactional publication, or Checkpoint 8 shared CRS centralization into that first point-cloud slice. Continue the remaining point-cloud decomposition before starting `city_models.py`.
