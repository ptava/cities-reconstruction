...
- Source baseline: stage-package reorganization is complete as one final review slice, with proposed final subject `refactor: organize pipeline stages into packages`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: the seventh behavior-preserving shapefiles decomposition delegates cross-source tree deduplication and configured polygon surface-precedence/overlap resolution to `stages/shapefiles/policy.py`. Shared GeoJSON-to-local-polygon conversion moves to `stages/shapefiles/transformation.py`; orchestration, gap filling, artifact writes, compatibility aliases, and the public `plan()`/`run()` facade remain stable in `stages/shapefiles/stage.py`. `.codegraph/` remains intentionally untracked.
- Completed review slice: every stage now has a package facade at `stages/<stage>/__init__.py` and `stages/<stage>/stage.py`; shapefiles implementation responsibilities are nested under `stages/shapefiles/`, and focused tests follow the matching `tests/stages/` hierarchy. The topology contract prevents a stage's tests from importing another stage's private `stage.py` module while allowing same-stage focused tests; `tests/test_artifacts.py` remains the explicit City4CFD-owner helper test. Ruff passed, configured mypy passed for 17 source files, full-package mypy passed for 40 source files, and pytest passed 457 tests.
...
| 3 | Break up god modules | In progress | The public shapefiles facade is now `stages/shapefiles/stage.py`, with diagnostics, input adapters/parsers, publication orchestration, HTML/SVG rendering, Markdown reporting, Overpass and supplemental transformation, and cross-source policy nested in the same package. `point_cloud/stage.py` remains 2,100 lines and `city_models/stage.py` 2,067. Preserve public `plan()`/`run()` facades while continuing focused slices. |
...
Status: presentation extraction committed in `b4be077`; pure diagnostics extraction committed in `2f02431`; publication extraction committed in `3cda1e9`; input-boundary extraction committed in `5681e3d`; Overpass transformation extraction committed in `ad4a983`; supplemental ESRI transformation extraction committed in `e702264`; cross-source policy extraction committed as `refactor: extract shapefiles cross-source policy`; stage-package reorganization is complete as one final review slice with proposed final subject `refactor: organize pipeline stages into packages`. The planned focused shapefiles slices are complete; decomposition of `point_cloud/stage.py` and `city_models/stage.py` has not started.
...
- Remaining Checkpoint 4 scope: begin `point_cloud/stage.py` with a behavior-preserving extraction of Markdown reporting and HTML/scene-data rendering, then continue its input, raster-processing, geometry, diagnostics, and publication boundaries in later focused slices before starting `city_models/stage.py`. Checkpoint 4 is not complete when the shapefiles work alone is finished.
...
Continue Checkpoint 4 with a behavior-preserving extraction of point-cloud Markdown reporting and HTML/scene-data rendering into focused modules. Keep `point_cloud/stage.py` input validation, raster processing, diagnostics, publication, external behavior, and public `plan()`/`run()` stable. Do not mix later point-cloud boundaries, Checkpoint 5 transactional publication, or Checkpoint 8 shared CRS centralization into that first point-cloud slice. Continue the remaining point-cloud decomposition before starting `city_models/stage.py`.
