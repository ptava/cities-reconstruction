...
- Source baseline: point-cloud alignment-diagnostics extraction is the current review slice, with proposed subject `refactor: extract point-cloud diagnostics`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: point-cloud projected-polygon predicates and spatial indexing, paired-raster scanning, and building/tree/unclassified point classification live in `stages/point_cloud/geometry.py`. Commit: `e31ec90 refactor: extract point-cloud geometry`.
- Current review slice: `stages/point_cloud/diagnostics.py` owns alignment status/message construction, deterministic integer-meter shift scoring, and the complete diagnostics payload. `stage.py` retains input-policy validation and handoff selection, CRS projection, artifact writes, manifest publication, and public `plan()`/`run()` behavior. The behavior-preserving verification passes 462 tests with 86.06% branch coverage on Python 3.11.12; Ruff passes, configured mypy passes for 17 source files, and full-package mypy passes for 45 source files. `.codegraph/` remains intentionally untracked.
...
| 3 | Break up god modules | In progress | The shapefiles decomposition is complete. Point-cloud input adapters, geometry/classification, diagnostics, reporting, and HTML/scene-data rendering now live in focused sibling modules, reducing `point_cloud/stage.py` to 602 lines; `city_models/stage.py` remains 2,067. Preserve public `plan()`/`run()` facades while completing the point-cloud publication slice. |
...
Status: the planned focused shapefiles slices and stage-package reorganization are complete. Point-cloud alignment-diagnostics extraction is the current review slice with proposed subject `refactor: extract point-cloud diagnostics`; decomposition of `city_models/stage.py` has not started.
...
- Current point-cloud diagnostics slice: `stages/point_cloud/diagnostics.py` owns alignment status/message construction, deterministic shift scoring, and the complete alignment/tree-filter diagnostics payload. `stage.py` calls `build_alignment_diagnostics()` after geometry classification, then retains artifact writes, manifest publication, and stable `plan()`/`run()` behavior. Proposed commit: `refactor: extract point-cloud diagnostics`.
- Remaining Checkpoint 4 scope: extract point-cloud publication next before starting `city_models/stage.py`.
...
After approval and commit of the current diagnostics slice, complete the point-cloud part of Checkpoint 4 with a behavior-preserving extraction of manifest/artifact publication. Keep input adapters, geometry/classification, diagnostics, artifact writes, external behavior, and public `plan()`/`run()` stable. Do not mix Checkpoint 5 transactional-publication expansion or Checkpoint 8 shared CRS centralization into that slice. Finish point-cloud decomposition before starting `city_models/stage.py`.
