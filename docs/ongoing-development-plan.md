...
- Source baseline: point-cloud manifest/artifact publication extraction is the current review slice, with proposed subject `refactor: extract point-cloud publication`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: point-cloud alignment status/message construction, deterministic integer-meter shift scoring, and the complete diagnostics payload live in `stages/point_cloud/diagnostics.py`. Commit: `62c968b refactor: extract point-cloud diagnostics`.
- Current review slice: `stages/point_cloud/publication.py` owns the typed publication input, stable artifact ordering and naming, optional tree handoff, metrics/details assembly, and the final manifest publication call. `stage.py` retains input-policy validation and handoff selection, CRS projection, artifact writes, orchestration, and public `plan()`/`run()` behavior. The behavior-preserving verification passes 464 tests with 86.11% branch coverage on Python 3.11.12; Ruff passes, configured mypy passes for 17 source files, and full-package mypy passes for 46 source files. `.codegraph/` remains intentionally untracked.
...
| 3 | Break up god modules | In progress | The shapefiles decomposition is complete. Point-cloud input adapters, geometry/classification, diagnostics, reporting, HTML/scene-data rendering, and publication now live in focused sibling modules, reducing `point_cloud/stage.py` to 569 lines; `city_models/stage.py` remains 2,067. Preserve public `plan()`/`run()` facades while starting the City4CFD presentation slice after approval and commit. |
...
Status: the planned focused shapefiles slices and stage-package reorganization are complete. Point-cloud manifest/artifact publication extraction is the current review slice with proposed subject `refactor: extract point-cloud publication`; decomposition of `city_models/stage.py` has not started.
...
- Current point-cloud publication slice: `stages/point_cloud/publication.py` owns the typed publication input, stable artifact ordering and naming, optional tree handoff, metrics/details assembly, and the final manifest publication call. `stage.py` invokes `publish_point_cloud_manifest()` only after every artifact write completes, while retaining stable orchestration and public `plan()`/`run()` behavior. Proposed commit: `refactor: extract point-cloud publication`.
- Remaining Checkpoint 4 scope: after approval and commit of point-cloud publication, begin decomposition of `city_models/stage.py` with its Markdown reporting and HTML/mesh scene-data rendering boundary.
...
After approval and commit of the current point-cloud publication slice, continue Checkpoint 4 with a behavior-preserving City4CFD presentation extraction: move Markdown report generation to `stages/city_models/reporting.py` and self-contained HTML plus mesh/scene-data rendering to `stages/city_models/rendering.py`. Keep input and handoff validation, City4CFD configuration and execution, geometry/diagnostics, artifact writes and publication, external behavior, and public `plan()`/`run()` stable. Do not mix later City4CFD boundaries, Checkpoint 5 transactional-publication expansion, or Checkpoint 8 shared CRS centralization into that slice.
