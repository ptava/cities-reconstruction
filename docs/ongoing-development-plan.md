...
- Source baseline: point-cloud presentation extraction is the current review slice, with proposed subject `refactor: extract point-cloud presentation`; inspect the live checkout and Git history for the aligned source, test, quality-gate, and documentation changes.
...
- Most recently completed code slice: point-cloud Markdown generation now lives in `stages/point_cloud/reporting.py`; self-contained HTML and browser scene-data preparation, including voxel preview sampling, now live in `stages/point_cloud/rendering.py`. The stage supplies its projected bounding box and tree-classification display thresholds explicitly, while input validation, CRS projection, raster processing, diagnostics, artifact writes, manifest publication, and public `plan()`/`run()` behavior remain in `stages/point_cloud/stage.py`. `.codegraph/` remains intentionally untracked.
- Current review slice: point-cloud presentation is extracted into focused reporting and rendering modules. The behavior-preserving verification passes 459 tests with 85.92% branch coverage on Python 3.11.12; Ruff passes, configured mypy passes for 17 source files, and full-package mypy passes for 42 source files.
...
| 3 | Break up god modules | In progress | The shapefiles decomposition is complete. Point-cloud reporting and HTML/scene-data rendering now live in focused sibling modules, reducing `point_cloud/stage.py` to about 1,450 lines; `city_models/stage.py` remains 2,067. Preserve public `plan()`/`run()` facades while continuing point-cloud input, raster, geometry, diagnostics, and publication slices. |
...
Status: the planned focused shapefiles slices and stage-package reorganization are complete. Point-cloud presentation extraction is the current review slice with proposed subject `refactor: extract point-cloud presentation`; decomposition of `city_models/stage.py` has not started.
...
- Completed point-cloud presentation slice: `stages/point_cloud/reporting.py` owns Markdown generation and `stages/point_cloud/rendering.py` owns self-contained HTML, scene payloads, voxel preview sampling, and footprint elevation lookup. `stage.py` retains orchestration and passes the already-derived projected bounding box and classification display thresholds into rendering. Verification passes 459 tests with 85.92% branch coverage on Python 3.11.12, Ruff, configured mypy (17 files), and full-package mypy (42 files).
- Remaining Checkpoint 4 scope: extract the point-cloud input and raster-reading boundary next, then continue geometry/classification, diagnostics, and publication in later focused slices before starting `city_models/stage.py`.
...
Continue Checkpoint 4 with a behavior-preserving extraction of the point-cloud input and raster-reading boundary. Keep geometry/classification, diagnostics, publication, external behavior, and public `plan()`/`run()` stable. Do not mix later point-cloud boundaries, Checkpoint 5 transactional publication, or Checkpoint 8 shared CRS centralization into that slice. Continue the remaining point-cloud decomposition before starting `city_models/stage.py`.
